...
export function WatermarkControlPanel() {
  ...
  const LayoutButton = ({ mode, icon: Icon, title }: { mode: WatermarkLayoutMode, icon: any, title: string }) => (
    <button
      ...
      className={`p-1.5 rounded transition-colors ${currentLayout === mode
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between pb-2 border-b border-white/10">
        <h3 className="text-sm font-medium">{t('水印设置')}</h3>
      ...
      {watermark.enabled && (
        <div className="space-y-5 animate-in fade-in slide-in-from-top-2">
          ...
          <div className="space-y-2">
            <div className="text-xs text-gray-400">{t('图文排版')}</div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs text-gray-400">
              <span className="flex items-center gap-1"><ImageIcon size={12} /> {t('图片/Logo')}</span>
            </div>
            ...
            <div className="relative group rounded-lg overflow-hidden border border-white/10 bg-black/40 h-20 flex items-center justify-center">
              {watermark.imageUrl ? (
                ...
                >
                  <span className="text-[10px]">{t('当前使用默认 Logo')}</span>
          ...
          <div className="space-y-3 pt-2 border-t border-white/5">
            ...
            <div className="flex gap-2">
              <button
                onClick={toggleBold}
                className={`flex-1 py-1.5 rounded flex items-center justify-center transition-colors ${(watermark.fontWeight || 400) >= 600 ? 'bg-accent-purple text-white' : 'bg-white/5 text-gray-400 hover:text-white'
                  }`}
                title={t('粗体')}
              >
              ...
              <button
                ...
                className={`flex-1 py-1.5 rounded flex items-center justify-center transition-colors ${watermark.fontStyle === 'italic' ? 'bg-accent-purple text-white' : 'bg-white/5 text-gray-400 hover:text-white'
                  }`}
                title={t('斜体')}
              >
                <Italic size={14} />
              ...
              <button
                ...
                className={`flex-1 py-1.5 rounded flex items-center justify-center transition-colors ${watermark.textDecoration?.includes('underline') ? 'bg-accent-purple text-white' : 'bg-white/5 text-gray-400 hover:text-white'
                  }`}
                title={t('下划线')}
              >
                <Underline size={14} />
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[10px] text-gray-500 mb-1 block">{t('文字颜色')}</label>
              ...
              <div>
                <label className="text-[10px] text-gray-500 mb-1 block">{t('背景颜色')}</label>
                <div className="flex items-center gap-2 bg-black/20 rounded p-1 border border-white/5 h-8">
                  <input
                    type="color"
          ...
          <div className="space-y-3 pt-2 border-t border-white/5">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs text-gray-400">
                <span>{t('不透明度')}</span><span>{watermark.opacity}%</span>
              </div>
              <input
                type="range"
                min="10"
