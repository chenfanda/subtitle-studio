  ...
  return (
    <div className="h-full flex flex-col bg-bg-primary">
      <div className="p-4 border-b border-border-secondary">
        <h3 className="text-lg font-semibold text-text-primary mb-1">{t('B-roll素材')}</h3>
          ...
          <div className="p-4 space-y-3">
            {subtitles.map((subtitle) => {
              ...
              return (
                <div
                  key={subtitle.id}
                  className={`flex gap-3 p-2 rounded-lg border-2 transition-all ${isSelected
                ...
                >
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-16 rounded overflow-hidden border-2 transition-all ${hasBroll
                      ? 'border-orange-500 shadow-lg shadow-orange-500/20 hover:border-red-500'
                      : 'border-border-secondary hover:border-accent-purple'
                      }`}
                    title={hasBroll ? t('点击删除 B-roll') : t('点击添加 B-roll')}
                  >
